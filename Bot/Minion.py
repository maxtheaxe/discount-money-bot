...
def collect_details():
    ...
    user_details = []
    user_details.append(
        input("\tEnter your first name for checkout purposes\n\t(press Enter to continue)\n\n\t"))
    user_details.append(
...
def launch(headless = False, signin_details = None):
    """
    Launch sequence to get driver started, bring to login page
    Args:
        headless ():
        signin_details ():
...
def get_formatted_time() -> datetime.strftime:
    """
    Return datetime with necessary twint formatting
    
    Returns: datetime.strftime
...
def restock_checker(last_check_time = "2020-12-23 14:20:00") -> tuple:
    """
    checks @tendmoney twitter for restock notifications
    
    Args:
        last_check_time ():

    Returns: tuple(bool, datetime)
...
def buy_money(driver, lowest_amount = None, product_ids = None, specific_target = None) -> None:
    """
    buys biggest quantity of money above given threshold, unless given target
    ...
    References:
        - https://stackoverflow.com/a/23078602/4513452
...
def checkout(driver, user_details) -> None:
    """
    check out with money
